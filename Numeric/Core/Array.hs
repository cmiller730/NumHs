...
linspace :: Double -> Double -> Int -> Array (S D) DIM1 Double
linspace !start !stop !num
  | num < 0 = hintSmall $ R.fromFunction (Z:.0) (\_ -> 0)
  | num == 1 = hintSmall $ R.fromFunction (Z:.1) (\_ -> start)
  | otherwise = let delta = (stop - start)/(fromIntegral num - 1)
                    fun :: DIM1 -> Double
                    {-# INLINE fun #-}
                    fun (Z:.idx) = start + (fromIntegral idx)*delta
                in hintSmall $ R.fromFunction (Z:.num) fun               
                  
logspace :: Double -> Double -> Double -> Int -> Array (S D) DIM1 Double
logspace !start !stop !base !num = 
  let x0 = linspace start stop num
  in hintSmall $ R.map (base**) x0 
     
arange :: Double -> Double -> Double -> Array (S D) DIM1 Double
...
eye :: Int -> Array (S D) DIM2 Double
eye !n | n < 0 = throw EyeNegativeDimension
       | otherwise = hintSmall $ R.fromFunction (Z:.n:.n) delta
        where 
          delta :: DIM2 -> Double
          {-# INLINE delta #-}
          delta (Z:.i:.j) = if (i==j) then 1 else 0

scalarInit :: (Shape sh) => sh -> Double -> Array (S D) sh Double
scalarInit !sh !x = hintSmall $ R.fromFunction sh (\_ -> x)

zeros :: (Shape sh) => sh -> Array (S D) sh Double 
zeros !sh = scalarInit sh 0.0

ones :: (Shape sh) => sh -> Array (S D) sh Double
